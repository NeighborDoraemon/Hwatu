...
public class Bear : Enemy_Parent, Enemy_Interface, Enemy_Stun_Interface
{
    [Header("Float Values")]
    ...
    private bool is_Attack_Call_Once = false; // 공격을 한번만 호출하기 위한 변수
    ...
    void FixedUpdate()
    {
        ...
        if (!BR_Stunned.Value)
        {
            if ((BR_Chasing.Value && Distance <= FR_Attack_Range.Value) || is_Attacking)
            {
                ...
                if (!is_Attack_Call_Once)
                {
                    Attack_Call();
                    is_Attack_Call_Once = true; // 공격을 한번만 호출하도록 설정
                }
            }
    ...
    private void Attack_Call()
    {
        if (!is_Attack_Turn) // 공격 시작 시, 플레이어 방향 보게하기
        {
            ...
            attackRoutine = StartCoroutine(Attack_Coroutine());
        }
    }
    ...
    private IEnumerator Attack_Coroutine()
    {
        ...
        yield return new WaitForSeconds(f_Before_Delay);

        Axe_Animator.SetTrigger("is_Attacking");
        is_Attack_Complete = true;

        yield return new WaitForSeconds(f_After_Delay);
        ...
        TurnAround();
        is_Attack_Call_Once = false; // 공격을 다시 호출할 수 있도록 설정
    }
